...
def plot_figB(results_dir: Path, out: Path, top_n: int = 10):
    """Barras horizontales agrupadas: importancia de genes por antibiótico."""
    groups = {
    ...
    for ax, (ab, tsv_path) in zip(axes, groups.items()):
        ...
        ax.invert_yaxis()
        ax.set_xlabel('Importancia (GBT)', fontsize=10)
        ax.set_title(f'{ab}\nTop {top_n} genes predictores',
